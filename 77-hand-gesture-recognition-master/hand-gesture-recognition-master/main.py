...
from action_recognition.ctcdecoder import BestPathDecoder
...
from action_recognition.spatial_transforms import (
    ...
    GaussCrop, HorizontalFlip, RandomRotation, MultiScaleCrop, Normalize, RandomHorizontalFlip,
    RandomContrast, RandomBrightness, RandomGaussianNoise, Scale, ToTensor
)
from action_recognition.target_transforms import ClassLabel
from action_recognition.temporal_transforms import (
    LoopPadding, TemporalRandomCrop, TemporalStride, TemporalRandomScale, TemporalRandomShift)
...
def export_onnx(args, model, export_name):
    ...
    x = param.new_zeros(1, args.sample_duration, 3, args.sample_size[0], args.sample_size[1])
...
def setup_dataset(args, train=True, val=True, test=True):
    ...
    to_tensor = ToTensor(args.max_pixel_value, args.min_pixel_value)

    train_spatial_transform = [Compose([
        Scale(max(args.sample_size) * 8 // 7),
        RandomRotation(args.sample_rotation),
        MultiScaleCrop(args.sample_size, args.scales),
        RandomGaussianNoise(args.gauss_noise, args.max_pixel_value, args.min_pixel_value),
        RandomContrast(args.contrast, args.max_pixel_value, args.min_pixel_value),
        RandomBrightness(args.brightness, args.max_pixel_value, args.min_pixel_value),
        to_tensor,
        normalization,
    ])]
    ...
    train_temporal_transform = Compose(
        [temporal_stride,
         TemporalRandomScale(args.temporal_scale),
         TemporalRandomShift(args.temporal_shift, args.temporal_shift_mode),
    ...
    # test data
    test_sample_size = int(max(args.sample_size) / args.scale_in_test)
    test_spatial_transform = [Compose([
        Scale(test_sample_size),
        CenterCrop(args.sample_size),
        to_tensor, normalization
    ])]
...
def configure_paths(args):
    relative_paths = ('video_path', 'annotation_path', 'flow_path')
...
def main():
    ...
    if args.ctc_loss:
        # init CTC decoder for testing
        decoder = BestPathDecoder(args.n_classes)
    else:
        decoder = None
    ...
    if args.train:
        ...
        train(args, model, train_loader, val_loader, criterion, optimizer, scheduler, args.logger, decoder)

    if not args.train and args.val:
        with torch.no_grad():
            validate(args, args.begin_epoch, val_loader, model, criterion, args.logger, decoder)
    ...
    if args.test:
        ...
        with torch.no_grad():
            with args.logger.scope():
                test(args, test_loader, model, args.logger, decoder)
