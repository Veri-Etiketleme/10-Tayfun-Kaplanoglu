...
cifar_model_files = {
    'mobilenet_v2': './data/checkpoints/mobilenet_v2_cifar10.pth',
}
...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, width_mult=1.0, inverted_residual_setting=None,
                 round_nearest=8, num_channels=3, input_size=(224, 224)):
        """
        ...
        if min(input_size) // 32 < 3:
            ...
            inverted_residual_setting[1][3] = 1
        else:
        ...
        self.feature_channels = _make_divisible(feature_channels * max(1.0, width_mult), round_nearest)
        features = [ConvBNReLU(num_channels, input_channel, stride=first_layer_stride)]
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            self.feature_size = tuple(in_size // s for in_size in self.feature_size)

        # building last several layers
        features.append(ConvBNReLU(input_channel, self.feature_channels, kernel_size=1))
...
def load_pretrained_model(model, arch, average_pretrained_input_weights=True, **kwargs):
    ...
    adapt_network_input_channels(pretrained_weights, 'features.0.0.weight',
                                 model.features[0][0].in_channels,
...
def mobilenet_v2(pretrained=False, **kwargs):
    ...
    if pretrained:
        model = load_pretrained_model(model, 'mobilenet_v2', **kwargs)
    return model
