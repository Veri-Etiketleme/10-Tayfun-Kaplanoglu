...
class AttentionLSTM(nn.Module):
    ...
    def __init__(self, input_features, hidden_size, attention_size, batch_first=False, use_gru=False,
        ...
        if use_gru:
            self.lstm = nn.GRU(input_features, hidden_size, batch_first=False, bidirectional=bidirectional, **kwargs)
        else:
            self.lstm = nn.LSTM(input_features, hidden_size, batch_first=False, bidirectional=bidirectional, **kwargs)

        self.attention = Attention(hidden_size * 2 if bidirectional else hidden_size, None, attention_size)
    ...
    def forward(self, x, hidden):
        ...
        for i in range(x.size(0)):
            ...
            hx = hidden if self.use_gru else hidden[0]
            if self.bidirectional:
                # for bidirectional: 2xNxL -> 1xNx2*L
                hx = hx.transpose(0, 1).reshape(1, hx.size(1), -1)
            feature, attention = self.attention(hx[0], v, v)
            feature = feature.transpose(0, 1)  # back to (N x C)

            # unsqueeze to emulate sequence size = 1
            _, hidden = self.lstm(feature.unsqueeze(0), hidden)
            output = hidden if self.use_gru else hidden[0]
            if self.bidirectional:
                # for bidirectional: 2xNxL -> 1xNx2*L
                output = output.transpose(0, 1).reshape(1, output.size(1), -1)
            outputs.append(output)
        # if bidirectional: first cat dim 0
