...
def resnet(X, is_training):
    ...
    d1 = tf.layers.dropout(h1, rate=0.5, training=is_training,name='D1')
    h2 = tf.layers.dense(d1, 128, activation=tf.nn.relu, use_bias=False, name='FC1')
    d2 = tf.layers.dropout(h2, rate=0.5, training=is_training, name='D2')
    h3 = tf.layers.dense(d2, 64, activation=tf.nn.relu, use_bias=False, name='FC2')
    d3 = tf.layers.dropout(h3, rate=0.5, training=is_training, name='D3')
    h4 = tf.layers.dense(d3, 32, activation=tf.nn.relu, use_bias=False, name='FC3')
    y = tf.layers.dense(h4, 2, activation=None, use_bias=False, name='Out')
    return y
