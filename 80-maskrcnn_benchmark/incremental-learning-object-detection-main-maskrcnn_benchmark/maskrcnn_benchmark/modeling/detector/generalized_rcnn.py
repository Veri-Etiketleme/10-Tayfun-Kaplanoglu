...
class GeneralizedRCNN(nn.Module):
    ...
    def forward(self, images, targets=None, rpn_output_source=None):
        """
        Arguments:
        ...
        if self.roi_heads:
            if self.training:
                x, result, detector_losses, roi_align_features = self.roi_heads(features, proposals, targets)
            else:
        ...
        if self.training:
            ...
            return losses, features, backbone_features, anchors, rpn_output, proposals, roi_align_features
    ...
    def generate_soften_proposal(self, images, targets=None):
        ...
        soften_scores, soften_bboxes, roi_align_features = self.roi_heads.calculate_soften_label(features, selected_proposals, targets)  # use ROI-subnet to generate final results

        return (soften_scores, soften_bboxes), selected_proposals, features, backbone_features, anchors, rpn_output, roi_align_features
