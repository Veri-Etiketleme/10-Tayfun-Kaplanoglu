...
class RPNHead(nn.Module):
    ...
    def __init__(self, cfg, in_channels, num_anchors):
        ...
        # check if the RPN network needs to be frozen
        if cfg.MODEL.RPN.CONV_FREEZE:
            print('rpn.py | freeze RPN convolutional layer')
            for name, param in self.conv.named_parameters():
                ...
                # print('requires_grad: {0}'.format(param.requires_grad))
        if cfg.MODEL.RPN.CLS_FREEZE:
            print('rpn.py | freeze RPN classification layer')
            for name, param in self.cls_logits.named_parameters():
                # print('parameter name: {0}, size: {1}'.format(name, param.size()))
                # print('requires_grad: {0}'.format(param.requires_grad))
                param.requires_grad = False
                # print('requires_grad: {0}'.format(param.requires_grad))
        if cfg.MODEL.RPN.BBS_FREEZE:
            print('rpn.py | freeze RPN bounding box layer')
...
class RPNModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, rpn_output_source=None):
        """
        Arguments:
    ...
    def feature_extraction(self, features):
        logits = self.head.feature_extraction(features)
